      ...
      u.enc.Base64 = {
         stringify: function (d) {
            ...
            for (var r = 0; r < p; r += 3)
               for (
                  var w =
                        (((l[r >>> 2] >>> (24 - 8 * (r % 4))) & 255) << 16) | (((l[(r + 1) >>> 2] >>> (24 - 8 * ((r + 1) % 4))) & 255) << 8) | ((l[(r + 2) >>> 2] >>> (24 - 8 * ((r + 2) % 4))) & 255),
                     v = 0;
                  4 > v && r + 0.75 * v < p;
                  v++
               )
...
(() => {
   ...
   try {
      ...
      if (v.method === "--en") {
         fs.writeFileSync(v.output, AES.Encrypt(contentInput, v.secret));
      } else if (v.method === "--de") {
         fs.writeFileSync(v.output, AES.Decrypt(contentInput, v.secret));
      }
   } catch (error) {
      v.error = error;
